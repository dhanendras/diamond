...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets1/assets/asset/'+assetID+'/owner', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	{		
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			result.message = asset.owner;
			tracing.create('EXIT', 'GET blockchain/assets1/assets/asset/'+assetID+'/owner', result);
			res.send(result)
		}
		else
		{
			...
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets1/assets/asset/'+assetID+'/owner', error)
			res.send(error)
		}
	})
