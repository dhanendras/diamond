...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets1/assets/asset/'+assetID+'/scrap', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		...
		{
			var result = {}
			var Diamond = JSON.parse(body.result.message);
			result.message = asset.scrapped;
			tracing.create('EXIT', 'GET blockchain/assets1/assets/asset/'+assetID+'/scrap' , result);
			res.send(result)
		}
		else 
		{
			...
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets1/assets/asset/'+assetID+'/scrap', error)
			res.send(error)
		}
	});
}
