$(document).ready(function(){
	...
	$("#subPg").click(function(){ //Transfer button
		...
		/*
		Formats the transaction request for a transfer of a V5C. Needs to have at least 1 assetID to transfer and a specified recipient.
		Constructs the transaction complete pop up at this point as well but only shows it once the transaction is complete.
		*/
		setCookie()
		//var table = document.getElementById("selDiamondsTbl");
		var index = 1;
		var id;
		var last = false;
		...
		if($('#selVhclsTbl tr').length >= 1 && $("#recipientInfo").html() != "")
		{
			...
			var spans = '';
			for(var i = 0; i < $('#selDiamondsTbl tr').length; i++)
			{
	...
	$("#scrapPg").click(function(){
		...
		*/
		...
		//var table = document.getElementById("selDiamondTbl");
		var index = 1;
		var id;
		var last = false;
		...
		if($('#selDiamondTbl tr').length >= 1)
		{
			var spans = '';
			for(var i = 0; i < $('#selDiamondTbl tr').length; i++)
			{
				spans += '<div class="loaderSpan" id="span'+(i+1)+'"><u>Asset '+(i+1)+'</u><br /><br /><span id="msg'+(i+1)+'Part1"></i>waiting...</i></span><br /><span id="msg'+(i+1)+'Part2"></span><br /><span id="msg'+(i+1)+'Part3"></span><br /><span id="msg'+(i+1)+'Part4"></span><br /></div>&nbsp;';
			}
			...
			$("#selDiamondTbl tr").each(function()
			{
				
				var assetIDAddr = $(this).find('.assetID').val();
				if(index == $('#selDiamondsTbl tr').length)
				{
					last = true;
				}
				$('#confTxt').html('Transaction committed to the blockchain. <br /><br />'+sendDets+'<br /><br />Assets: '+$('#selDiamondsTbl tr').length)
