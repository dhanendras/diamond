...
function createAsset()
{
    ...
    xhr.open('POST', '/blockchain/assets1/assets', true);
...
function transferAsset()
{
    let found = [];
    let xhr = new XMLHttpRequest();
    xhr.open('PUT', '/blockchain/assets1/assets/'+transferArray[transferIndex-1].assetID+'/owner', true); //PUT (Updates)
    xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
    xhr.overrideMimeType('text/plain');
    xhr.onprogress = function () {
        let data = xhr.responseText;
...
function scrapAsset()
{
	...
	/*
	Scraps the chosen asset, specified by assetIDAddr. Because we want to ensure the AssetID identifier or the VIN is never assigned to another contract, we
	don't actually delete the contract, instead we update the scrapped attribute to be true.
	*/
    let found = [];
    let objects = [];
    let xhr = new XMLHttpRequest();
    xhr.open('DELETE', 'blockchain/assets1/assets/'+scrapArray[scrapIndex-1].v5cID, true); //As it refers to an v5, the path is: blockchain/assets1/assets/asset/..
...
let bigData;
let increment;
function updateAsset(clarity, cut, date, colour, diamondat, jewellery_type, location, symmetry, polish, timestamp el)
{
	/*
	Formats the transaction request to update an attribute of a AssetID. The logic on who can update what is contained within the contract on the blockchain.
	*/
    ...
    bigData = [{'value':clarity,'field':'clarity', 'title': 'clarity','assetID':assetID},{'value':cut,'field':'cut', 'title': 'cut','v5cID':v5cID},{'value':diamondat,'field':'diamondat', 'title': 'diamondat','v5cID':v5cID},{'value':colour,'field':'colour', 'title': 'Colour','v5cID':v5cID},{'value':date,'field':'date', 'title': 'date','v5cID':v5cID}],{'value':location,'field':'location', 'title': 'location','v5cID':v5cID},{'value':symmetry,'field':'symmetry', 'title': 'symmetry','v5cID':v5cID}],{'value':polish,'field':'polish', 'title': 'polish','v5cID':v5cID}],{'value':time_stamp,'field':'time_stamp', 'title': 'time_stamp','v5cID':v5cID}],{'value':jewellery_type,'field':'jewellery_type', 'title': 'jewellery_type','v5cID':v5cID}],
    increment=0;
...
function updateField()
{
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('PUT', '/blockchain/assets1/assets/'+bigData[increment].assetID+'/'+bigData[increment].field, true);
    xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
    xhr.overrideMimeType('text/plain');
    xhr.onprogress = function () {
        let data = xhr.responseText;
